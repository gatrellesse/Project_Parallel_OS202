...
# Cabeçalho do CSV
echo "ENV_VALUE,n_iterations,avg_global_time,avg_time_update,avg_time_affichage" > "$CSV_FILE"
...
for val in "${ENV_VALUES[@]}"; do
  ...
  for ((run=1; run<=N_RUNS; run++)); do
    ...
    full_output=$(export $ENV_VEAR_NAME=$val && $EXECUTABLE_PATH -n 100 -v $val)
  ...
  # Adiciona ao CSV
  echo "$val,$n_iter,$avg_global,$avg_update,$avg_affichage" >> "$CSV_FILE"
done
